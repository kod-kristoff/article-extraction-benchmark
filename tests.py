...
from evaluate import _ngrams, _tokenize, load_prediction, string_shingle_matching
from extractors.output_util import cargo_lock_package_version, go_mod_dep_version


def test_tokenize():
    assert _tokenize("a b,cd:e(foo,bar) ") == ["a", "b", "cd", "e", "foo", "bar"]


def test_go_mod_dep_version() -> None:
    assert (
        go_mod_dep_version(
...
def test_cargo_lock_package_version() -> None:
    assert (
        cargo_lock_package_version(
            Path("extractors/rust_extractors/Cargo.lock"), "august"
        )
        == "2.4.0"
    )
